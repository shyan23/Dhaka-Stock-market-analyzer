...
class StockMarketApp:
    ...
    def _initialize_session_state(self):
        """Initialize Streamlit session state variables with persistent data (user-specific)"""
        # Get user prefix for session state keys
        user_prefix = self.auth_service.get_user_session_prefix()

        # User-specific session state keys
        selected_stocks_key = f'{user_prefix}selected_stocks'
        portfolio_items_key = f'{user_prefix}portfolio_items'
        transactions_key = f'{user_prefix}transactions'
        portfolio_settings_key = f'{user_prefix}portfolio_settings'
        cash_balance_key = f'{user_prefix}cash_balance'
        cash_transactions_key = f'{user_prefix}cash_transactions'

        # Initialize user-specific data
        if selected_stocks_key not in st.session_state:
            st.session_state[selected_stocks_key] = self._load_selected_stocks()

        if portfolio_items_key not in st.session_state:
            st.session_state[portfolio_items_key] = self._load_portfolio_items()

        if transactions_key not in st.session_state:
            st.session_state[transactions_key] = self._load_transactions()

        if portfolio_settings_key not in st.session_state:
            st.session_state[portfolio_settings_key] = self._load_portfolio_settings()

        if cash_balance_key not in st.session_state:
            st.session_state[cash_balance_key] = st.session_state[portfolio_settings_key].get('initial_fund', 100000.0)

        if cash_transactions_key not in st.session_state:
            st.session_state[cash_transactions_key] = []
        ...
        st.session_state.selected_stocks = st.session_state[selected_stocks_key]
        st.session_state.portfolio_items = st.session_state[portfolio_items_key]
        st.session_state.transactions = st.session_state[transactions_key]
        st.session_state.portfolio_settings = st.session_state[portfolio_settings_key]
        st.session_state.cash_balance = st.session_state[cash_balance_key]
        st.session_state.cash_transactions = st.session_state[cash_transactions_key]
    ...
    def _handle_authentication(self):
        """Handle user authentication and show login screen if needed"""
